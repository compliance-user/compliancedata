...
class Users(Resource):
	...
	def post(self, action_name=None):
		...
		if action_name == 'create_user':
			user_existence = db.user.find_one({
				'username': request.headers['User']
				})
			if not user_existence:
				if request.headers['User'] and request.headers['Password']:
					db.user.insert({'username': request.headers['User'],
	...
	def put(self, user_id, action_name=None):
		if user_id and action_name == 'update_user':
			check_user_exists = db.user.find_one({
				'_id': ObjectId(user_id)
				})
